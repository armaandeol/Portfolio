<script lang="ts">
	...
	export let items: (Content.BlogpostDocument | Content.ProjectDocument)[] = []; // Add default empty array
...
<ul on:mouseleave={onMouseLeave} class="grid border-b border-b-slate-100">
	{#if items && items.length > 0}
		...
		{/each}
	{:else}
		<li class="py-10 text-center text-slate-400">No items to display</li>
	{/if}
</ul>
